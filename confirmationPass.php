			...
			if ($check_unameMail_num >= 1) {
				$chengePass_cnfrm = "
							<form action='confirmationPass.php?u=".$unameMail."&code=".$uCodeMail."' method='POST'>
								<p>Change Your LGBT Password!</p></br>
			...
			if (isset($_POST['confrmChangepass'])) {
				if ($newpassword == $repear_password) {
					...
					if (mysql_query($updatePassQuery)) {
						...
						// send email
						$msg = "Peace Upon You  ".$get_first_nm_fetch_query."
								successfully your LGBT password has been changed.
								";
						mail($get_email_fetch_query,"LGBT Password Changed",$msg, "From:LGBT <no-reply@LGBT.com>");
					}else {
