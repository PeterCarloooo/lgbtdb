...
//check for userfrom propic delete
 ...
 <?php 
	//lgbt update and file check
	$error = "";
	$lgbt = $_POST['lgbt'];
	$lgbt = trim($lgbt);
	$lgbt = mysql_real_escape_string($lgbt);
	...
	if ($pic != "") {
		...
		if (((@$_FILES['uploadFile']['type']=='image/jpeg') || (@$_FILES['uploadFile']['type']=='image/png') || (@$_FILES['uploadFile']['type']=='image/gif')) && (@$_FILES['uploadFile']['size'] < 200000)) {
			...
			if (file_exists($photoLocat."/userdata/profile_pics/$chare/".@$_FILES["uploadFile"]["name"])) {
				...
				$sqlCommand = "INSERT INTO posts(lgbt_body,date_added,added_by,user_posted_to,photos,lgbt_give) VALUES('$lgbt', '$date_added','$added_by', '$user_posted_to','$photos','1')";
				...
				$added_by = $user;
				$user_posted_to = $user;
				$sqlCommand = "INSERT INTO posts(lgbt_body,date_added,added_by,user_posted_to,lgbt_give) VALUES('$lgbt', '$date_added','$added_by', '$user_posted_to','1')";
				$query = mysql_query($sqlCommand) or die (mysql_error());
				header("Location: index.php");
			}
			else if ($lgbt == "") {
			...
			<tbody>
				<form action="" method="POST"  enctype='multipart/form-data'>
					<tr style="margin-right: 10px;">
							<td>
								<div class="">
									<textarea type="text" id="post" name="lgbt" onkeyup="clean("post")" onkeydown="clean("post")" class="WpostForm_text" placeholder="What you are thinking..."></textarea>
					...
					<table style="width: 100%;">
						...
						<td>
							<input type='submit' name='uploadpic' value='lgbt' class='WpostSubmit' >
						</td>
					</table>
					</tr>
				</form>
