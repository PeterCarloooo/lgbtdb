...
<html>
<head>
	<title><?php echo $title_fname; ?> • LGBT Community</title>
			...
			$count = "";
				...
				echo '<div>
					     <table class="friendsmenu">
							<tr>
								<th style="background-color: #686B68;"><a href="friends.php?u='.$username.'"><h1>Friends </br>'.$count_frnd_num.'</h1></a></th>
					...
					if ($count_frnd_num == 0){
						echo '<div class="search_banner"><p style="font-size: 15px; margin: 10px; font-weight: bold; color: #7B7B7B;">No friends found!<br></p>
						</div>';
					}else {
						...
						while ($row=mysql_fetch_array($queryfollowing)) {
							...
							if ($pro_num == 0) {
								$profile_picuser_to = "http://www.lgbt.com/img/default_propic.png";
							}else {
								$pro_changed_db = $get_pro_changed['photos'];
								if ($pro_changed_db != $profile_picuser_to_db) {
									$profile_picuser_to = "http://www.lgbt.com/img/default_propic.png";
								}else {
									$profile_picuser_to = "http://www.lgbt.com/userdata/profile_pics/".$profile_picuser_to_db;
								}
							}
				...
				echo "<p style='text-align: center; color: #7B7B7B; margin: 10px; font-weight: bold; font-size: 20px;'>Sorry! Nothing to view. </p>";
		}
	}else {
