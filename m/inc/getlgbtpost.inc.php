<?php
	...
	$sharepostid = $row['lgbt_post'];
	$discription = $row['discription'];
	$photos_db = $row['photos'];
	$photos = "http://www.lgbt.com/userdata/profile_pics/".$photos_db;
	...
	if (strlen($body) > 3000) {
			$body = substr($body, 0, 3000)." <a href='viewPost.php?pid=".$id."' style='text-decoration: none; color: #0B810B;'>... read more</a>";
		}else {
			$body = $body;
			}
	...
	//showing data on profile
			...
			if ($user_posted_to == $added_by) {
					...
					echo "<div style='margin: 0px 10px;'><div class='posted_by'><a href='$added_by' title=\"Go to $add_by's Profile\">$add_by &nbsp;&nbsp;</a><h style='font-size: 13px; color: #656262;'>>>&nbsp;&nbsp;</h><a href='$user_posted_to' title=\"Go to $post_to_fname's Profile\">$post_to_fname</a> <span style='color: #9E9E9E; font-weight: normal;'>$discription</span></div>";
			}
			echo "<div class='posted_body'><div class='posted_date'>$date_added</div><p style='word-break: break-all;'>".nl2br($body)."</p>";
				...
				if($sharepostid != 0) {
					...
					$sp_photos = "http://www.lgbt.com/userdata/profile_pics/".$sp_photos_db;
				...
				echo "<a href='viewPost.php?did=".$id."' style=' margin-left: 10px;'>Comments ($count)</a>
