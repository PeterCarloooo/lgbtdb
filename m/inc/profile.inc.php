...
<?php 
	...
	//check for cover delete
		...
		if ($pro_num == 0) {
			$cover_pic= "http://www.lgbt.com/img/default_covpic.png";
		}else {
			$pro_changed_db = $get_pro_changed['photos'];
		if ($pro_changed_db != $cover_pic_db ) {
			$cover_pic= "http://www.lgbt.com/img/default_covpic.png";
		}else {
			$cover_pic= "http://www.lgbt.com/userdata/profile_pics/".$cover_pic_db ;
		}
		}
			...
			<div style="line-height: 1.3;">
				<?php 
					if ($verify_id_user == 'yes') {
						echo '<div><span style="font-size: 23px; font-weight: 800; color: #ffffff" >'.$first_name_user.'</span><span class="verifiedicon" style="background: url(http://www.lgbt.com/img/verifiLogo.png) repeat; background-size: cover !important;" title="Verified profile"></span></div>';
					}else {
