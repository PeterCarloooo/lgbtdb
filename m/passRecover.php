...
if (isset($_POST['searchId'])) {
	...
	if ($username_query_num >= 1) {
		...
		if ($get_block_fetch_query == 1 ) {
			...
			$_SESSION['pro_upics'] = $profile_pic;
			...
			$succes_echoEmail = "
				...
				<form action='passRecover.php' method='POST'>
					...
					<span style='font-size:  margin-left: 13px;  font-weight: 500; color: #575454'>@".$username."</span></br></br>
						Please enter your email to send code:</br></br><input type='email' name='recovEmail' class='pass_body2Input' size='30' required ></br>
					</hr>
			...
			$success_msg = "
				...
				<form action='passRecover.php' method='POST'>
					<p style='color: #4C9ED9; font-size: 14px;'><input type='radio' checked> Email confirmation code to: ".$username." </p></br>
...
if (isset($_POST['searchRecovEmail'])) {
	$profile_pic = $_SESSION['pro_upics'];
	...
	if ($searchRecovEmail_query_num >= 1) {
		...
		if (mysql_query($confirmCodeQuery)) {
			...
			";
			mail($recovEmail,"lgbt Confirmation Code",$msg, "From:lgbt <no-reply@lgbt.com>");
			$success_email_cnfrm = "
				<form action='passRecover.php' method='POST'>
						Verification Code:</br></br><input type='text' name='confirmMailCode' class='pass_body2Input' size='30' required ></br>
					</hr>
						New Password:</br></br><input type='password' name='newpassword' class='pass_body2Input' size='30' required></br>
					</hr>
						Retype Password:</br></br><input type='password' name='newpassword2' class='pass_body2Input' size='30' required></br>
					</hr>
...
if (isset($_POST['searchRecovEmail2'])) {
	...
	if ($searchRecovEmail_query_num >= 1) {
		...
		if (mysql_query($confirmCodeQuery)) {
			...
			N.B. If it's not by you, delete this mail or forget it.
			";
			mail($user_con_email,"lgbt Confirmation Code",$msg, "From:lgbt <no-reply@lgbt.com>");
			$success_email_cnfrm = "
				<form action='passRecover.php' method='POST'>
						Verification Code:</br></br><input type='text' name='confirmMailCode' class='pass_body2Input' size='30' required ></br>
					</hr>
			...
			if ($confirmMailCode == $get_confirmCode_fetch_query) {
				if ($newpassword == $repear_password) {
					...
					if (mysql_query($updatePassQuery)) {
						...
						// send email
						$msg = "Successfully your lgbt password has been changed. 
						
						If it's not by you, please follow the link.
				
						http://www.lgbt.com/confirmationPass.php?u=".$get_uName_fetch_query."&code=".$get_confirmCode_fetch_query ."
						
						";
						mail($get_uEmail_fetch_query ,"lgbt Password Changed",$msg, "From:lgbt <no-reply@lgbt.com>");
					}
				}else {
...
<head>
	...
	<link rel="icon" href="http://www.lgbt.com/img/title.png" type="image/x-icon">
	<meta charset="uft-8">
...
<div>
	<div><?php include ( "./inc/header.inc.php" ); ?></div></br>
	<div class="pass_body1">
			<?php echo $error; ?>
		<div class="pass_body2">
				<?php
					if (isset($success_msg)) {
						...
						echo '
							<form action="passRecover.php" method="POST">
								<p>Find your lgbt account!</p></br>
									Enter email or username:</br></br><input type="text" name="username" class="pass_body2Input" size="30" required></br>
								</hr></br>
