...
if (isset($_REQUEST['pid'])) {
	...
	$db_username = $get_file_name['added_by'];
	$u_posted_to= $get_file_name['user_posted_to'];
	$sharelgbtid = $get_file_name['lgbt_post'];
	if($sharelgbtid != 0) {
		$get_sharepost = mysql_query("SELECT * FROM posts WHERE id='$sharelgbtid'");
		$post_info = mysql_fetch_assoc($get_sharepost );
		$sp_id = $post_info['id'];
		$sp_body = $post_info['body'];
	}
...
}else if(isset($_POST['lgbt'])) {
	$user_posted_to = $user;
	$date_added = date("Y-m-d");
	$added_by = $user;
	
	$sqlCommand = "INSERT INTO posts(body,date_added,added_by,user_posted_to,lgbt_post) VALUES('$post', '$date_added', '$added_by', '$user_posted_to', '$id')";
	$query = mysql_query($sqlCommand) or die (mysql_error());
	header("Location: index.php");
}else {
			...
			echo '
				...
				<div style="width: 100%;">
					...
					if (isset($_REQUEST['pid'])) {
						...
						echo'<input name="lgbt" value="lgbt" type="submit" class="confirmSubmit"></input>';
					}
					echo'</div>
					<div class="cc">
