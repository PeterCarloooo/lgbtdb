...
<html>
<head>
	<title>View LGBT Community - LGBT Community</title>
	<meta charset="utf-8" />
...
//for getting post
$getposts = mysql_query("SELECT * FROM lgbt WHERE id ='$id'") or die(mysql_error());
...
while ($row = mysql_fetch_assoc($getposts)) {
		...
		$photos = "http://www.lgbt.com/userdata/lgbt_pics/".$photos_db;
		...
		//showing data on profile
				echo "<div class='postBody post_search_result_box'>";
						...
						if ($photos_db == "") {
							...
							echo "<a href='viewlgbt.php?pid=".$id."'><img src='$photos' style='max-width: 420px; width: 100%; border: 1px solid #ddd;'/></a>";
						}
						...
						if ($rows_uname == 1) {
							...
							echo "<a href='viewlgbt.php?pid=".$id."'>Comments ($count)</a>";
						}else if ($user != '') {
							echo "<a href='lgbtLike.php?did=".$id."' >Like ($count_like)</a>";
							echo "<a href='viewlgbt.php?pid=".$id."'>Comments ($count)</a>";
						}else {
					...
					<iframe style='width: 100%; height: auto; min-height: 300px;' src='./lgbt_cmntsFrame.php?id=$id' frameborder='0'></iframe>
					</div>";
		}
	}
 ?>
