...
<html>
<head>
	<title>Search • LGBT Community</title>
			...
			if (($_GET['keywords'] && $_GET['topic']) == NULL) {
				...
				if ($count == 0){
					...
					echo '<div class="search_banner">Result for: 
						...
						<tr style="font-weight: bold;" colspan="10" bgcolor="#000">
							<th>Id</th>
							<th>Full Name</th>
							<th>Username</th>
							<th>Email</th>
				...
				if ($count == 0){
					...
					$search_for = $search_value;
					$query = "SELECT id,body,date_added,added_by FROM lgbt where body like '%$search_value%' ORDER BY id DESC";
					$query = mysql_query($query) or die ("could not count");
					$count = mysql_num_rows($query);
				if ($count == 0){
					echo '<div class="search_banner">No match found!
