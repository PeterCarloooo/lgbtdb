	...
	<?php include ( "./inc/header.inc.php" ); ?>
		<div style="width: 900px; margin: 52px auto;">
			...
			<div style="float: right;">
				 <div class="notificationbox">
				 	<a href="notifications.php"><h2 style="float: left; color: #0B810B;">LGBT Notification&nbsp;|</h2></a><a href="notification.php"><h2 style="float: left;color: #848684;">&nbsp;Post Notification</h2></a>
				 			...
				 			<li style='line-height: 8px;'>
				 				<?php 
									...
									$count_notipost = mysql_num_rows($get_notipost);
				 					//getting lgbt comment
									$get_notiLGBT = mysql_query("SELECT * FROM lgbt_comments WHERE lgbt_to='$user' AND lgbt_by != '$user' ORDER BY id DESC LIMIT 50");
									$count_notiLGBT = mysql_num_rows($get_notiLGBT);
									$count = $count_notiLGBT + $get_notiLGBT;
									if ($count != 0) {
										//getting lgbt noti
										while ($noti_LGBT = mysql_fetch_assoc($get_notiLGBT)) {
											$LGBT_id = $noti_LGBT['LGBT_id'];
											$LGBT_body = $noti_LGBT['LGBT_body'];
											$time = $noti_LGBT['time'];
											$user_by = $noti_LGBT['LGBT_by'];
											$user_to = $noti_LGBT['LGBT_to'];
		...
		if ($pro_changed_db != $profile_pic_db) {
			...
			$profile_pic = "userdata/profile_pics/".$profile_pic_db;
											...
											$get_descrip = mysql_query("SELECT * FROM lgbt WHERE id='$lgbt_id'");
											$get_description = mysql_fetch_assoc($get_descrip);
											$discription = $get_description['photos'];
											if ($discription == "") {
												$comment_disp = "commented on your lgbt";
												$posted_pic = "";
											}else {
												$comment_disp = "commented on your lgbt photo";
												$posted_pic = "userdata/lgbt_pics/".$discription;
											}
											if ($discription == "") {
												$comment_disp = "commented on your lgbt";
											}else {
												...
												echo "<div style='margin: 0 0 0 50px; line-height: 1.4;'>";
													...
													"; 
														 if ($discription == "") {
														 	...
														 	echo "<a href='viewlgbt.php?did=".$LGBT_id."'><img src='$posted_pic' style= 'float: right;' height='55' width='65' /></a>";
														 }
													echo "<br>";
														echo "<span style='line-height: 1.5; color: #444;' >".nl2br($LGBT_body)."</span><br>
