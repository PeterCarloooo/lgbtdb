	...
	while ($row = mysql_fetch_assoc($password_query)) {
		...
		if ($oldpassword_md5 == $db_password) {
			if ($newpassword == $repear_password) {
				...
				// send email
				$msg = "Peace Upon You  ".$db_first_name."
				
				Successfully your LGBT password has been changed. 
						
						If it's not by you, please follow the link.
				
						http://www.lgbt.com/confirmationPass.php?u=".$user."&code=".$confirmCode."
						
						";
						mail($db_email ,"LGBT Password Changed",$msg, "From:LGBT <no-reply@lgbt.com>");
				}
			}else {
